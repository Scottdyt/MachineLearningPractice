...
def run_networks():
    ...
    for size in SIZES:
        ...
        # get data
        training_data, validation_data, test_data = Network2.load_data_wrapper()
        ...
        # train data
        net.SGD(training_data[:size], num_epochs, 10,
                0.5, lmbda=size*0.0001, printf=False)
        accuracy = net.accuracy(validation_data, convert=True) / 100.0
        print("Accuracy was %s percent" % accuracy)
        accuracies.append(accuracy)
    f = open("overfit\more_data.json", "w")
...
def make_plots():
    f = open("overfit\more_data.json", "r")
